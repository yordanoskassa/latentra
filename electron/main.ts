...
ipcMain.handle('composio:verifyConnection', async (event, connectionId: string) => {
  try {
    const apiKey = process.env.COMPOSIO_API_KEY
    if (!apiKey) {
      return { success: false, error: 'No API key configured' }
...
ipcMain.handle('agent:getById', async (event, id: string) => {
  try {
...
ipcMain.handle('agent:update', async (event, id: string, updates) => {
  try {
...
ipcMain.handle('agent:uploadFile', async (event, agentId: string) => {
  try {
...
ipcMain.handle('agent:getFiles', async (event, agentId: string) => {
  try {
...
ipcMain.handle('agent:deleteFile', async (event, fileId: string) => {
  try {
